...
def parse_arguments(args):
    ...
    parser.add_argument(
        '-ppx','--ptcproxy', type=str, default=None,
        help='Proxy used when creating accounts'
    )
    parser.add_argument(
...
def entry():
    ...
    if _verify_settings({'args':args, 'balance':captchabal}):
        ...
        for x in range(0,args.count):
            ...
            try:
                try:
                    account_info = pikaptcha.random_account(username, args.password, args.email, args.birthday, args.plusmail, args.recaptcha, args.captchatimeout, args.ptcproxy)
